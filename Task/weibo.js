...
暂时不用: https:\/\/m\.weibo\.cn\/c\/checkin\/ug\/v2\/signin\/signin url 
...
let tokenArr = [],infoArr = [],wbinfo;
let wbtoken = $.getdata('sy_token_wb')
//let wbinfo = $.getdata('info_wb')
...
if ($.isNode()) {
  ...
  if (process.env.WB_INFO && process.env.WB_INFO.indexOf('#') > -1) {
   wbinfo = process.env.WB_INFO.split('#');
  }
  else if (process.env.WB_INFO && process.env.WB_INFO.split('\n').length > 0) {
   wbinfo = process.env.WB_INFO.split('\n');
  } else  {
   wbinfo = process.env.WB_INFO.split()
  };
  ...
  Object.keys(wbtoken).forEach((item) => {
    ...
    //Object.keys(wbinfo).forEach((item) => {
        //if (wbinfo[item]) {
            //infoArr.push(wbinfo[item])
        //}
    //});
}
!(async () => {
...
function getsign() {
  return new Promise((resolve, reject) =>{
   let signurl =  {
      ...
      headers: {"User-Agent": `Weibo/52021 (iPhone; iOS 13.7; Scale/3.00)`}}
//$.log(signurl.url)
 ...
 return new Promise((resolve, reject) =>{
   $.post({
      ...
      headers:{
        ...
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Weibo (iPhone10,1__weibo__11.2.1__iphone__os14.2)'
       }
     }, (error, resp, data) => {
     let result = JSON.parse(data)
     if (result.status == 1){
